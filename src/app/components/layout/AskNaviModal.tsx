...
searchInput.addEventListener('input', debounce((e) => {
  ...
  if (askNaviContext === 'product_recommendation') {
    const recommendationReady = Boolean(recommendationAnswers.paymentType && recommendationAnswers.buildType);
    const choose = (key: string, value: string) => setRecommendationAnswers(prev => ({ ...prev, [key]: value }));
    const recommendationExplanation = recommendationAnswers.paymentType === 'Accept payments with QR' && recommendationAnswers.buildType === 'Integrate with an API'
      ? 'QR API looks like the right fit. You can generate dynamic QR payments directly from your application and receive payment updates through your integration.'
      : recommendationAnswers.paymentType === 'Accept payments in my website or app' && recommendationAnswers.buildType === 'Integrate with an API'
      ? 'We recommend QR API. It lets you add dynamic KHQR payments to your website or application and test the full payment flow in Sandbox.'
      : 'Based on what you\'ve told me, QR API is a good place to start. You can explore the Sandbox, generate test QR payments and learn how the integration works.';
    const startQrIntegration = () => {
      updateState({
